...
with lado_esq:
    ...
    with tab1:
        ...
        with col23:
            ...
            df_evolution_year_plot["date"] = df_evolution_year_plot["date"].astype(str).str[:7]
            df_evolution_year_plot = df_evolution_year_plot.pivot(index="date", columns="account", values="balance")

            st.bar_chart (df_evolution_year_plot, x_label="Month", y_label="Variation (€)", stack= True, height=520)
...
with lado_dir:
    ...
    with col1:
        ...
        with tab1:
            ...
            with tab12:
                df_income_by_cat_visual = df_income_by_cat.copy()
                df_income_by_cat_visual ["weight"] = df_income_by_cat_visual ["value"] / (df_income_by_cat_visual ["value"].sum())
                df_income_by_cat_visual = df_income_by_cat_visual.sort_values ("value", ascending = False)
                st.dataframe(df_income_by_cat_visual, hide_index=True, column_config={"value": st.column_config.NumberColumn ("Total Income (EUR)", format="euro"),
        ...
        with tab2:
            ...
            df_income_year = df_income_year.rename (columns = {"year": "Year",
            ...
            df_income_year_avg = df_income.copy()
            df_income_year_avg["year"] = df_income_year_avg["date"].dt.to_period("Y").astype(str)

            df_income_year_avg =  df_income_year_avg.groupby (["year", "secondary_category"]) ["value"].sum().reset_index()
            df_income_year_avg =  df_income_year_avg.groupby (["secondary_category"]) ["value"].mean().reset_index()
            df_income_year_avg["value"] = round (df_income_year_avg["value"], 2) 

            df_income_year_avg = df_income_year_avg.rename (columns = {"year": "Year",
                                                                       "secondary_category": "Secondary Category",
                                                                       "value": "Value"})
    ...
    with col2:
        ...
        with tab21:
            ...
            with tab11:
                ...
                df_primary_exp_plot = df_primary_exp.copy()
                df_primary_exp_plot ["value"] = df_primary_exp_plot ["value"] * (-1)

                fig, ax = plt.subplots()
                
                wedges, label_texts, autopct_texts = ax.pie(df_primary_exp_plot["value"], labels=df_primary_exp_plot["main_category"], autopct='%1.1f%%', startangle=40,
            ...
            with tab12:
                df_primary_exp_visual = df_primary_exp_plot.copy()
                df_primary_exp_visual ["weight"] = df_primary_exp_visual ["value"] / (df_primary_exp_visual ["value"].sum())
                df_primary_exp_visual = df_primary_exp_visual.sort_values ("value", ascending = False)
                st.dataframe(df_primary_exp_visual, hide_index=True, column_config={"value": st.column_config.NumberColumn ("Total Expense (EUR)", format="euro"),
        ...
        with tab22:
            ...
            df_primary_year_avg =  df_primary_year_avg.groupby (["year", "main_category"]) ["value"].sum().reset_index()
            df_primary_year_avg =  df_primary_year_avg.groupby (["main_category"]) ["value"].mean().reset_index()
            df_primary_year_avg["value"] = round (df_primary_year_avg["value"], 2) 
        ...
        with tab23:
            ...
            with tab11:
                ...
                df_primary_aggregated = df_primary_aggregated.copy()
            ...
            with tab12:
                df_primary_aggregated_visual = df_primary_aggregated.copy()
                df_primary_aggregated_visual ["weight"] = df_primary_aggregated_visual ["value"] / (df_primary_aggregated_visual ["value"].sum())
                df_primary_aggregated_visual = df_primary_aggregated_visual.sort_values ("value", ascending = False)
                st.dataframe(df_primary_aggregated_visual, hide_index=True, column_config={"value": st.column_config.NumberColumn ("Total Expense (EUR)", format="euro"),
        ...
        with tab24:
            ...
            df_primary_aggregated_year_avg =  df_primary_aggregated_year_avg.groupby (["year", "essential"]) ["value"].sum().reset_index()
            df_primary_aggregated_year_avg =  df_primary_aggregated_year_avg.groupby (["essential"]) ["value"].mean().reset_index()
            df_primary_aggregated_year_avg["value"] = round (df_primary_aggregated_year_avg["value"], 2) 
...
#     ################################################################################################################################################################################################
    ...
    with col101:
        ...
        df_secondary_exp_plot = df_secondary_exp_plot.groupby (["main_category", "secondary_category", "essential"]) ["value"].sum()
        df_secondary_exp_plot = df_secondary_exp_plot.reset_index()
    ...
    with col102:
        ...
        df_exp_fragm = df_secondary_exp.copy()
        ...
        df_exp_fragm_plot = df_exp_fragm [["secondary_category", "value"]]
        df_exp_fragm_plot = df_exp_fragm_plot.sort_values("value")
    ...
    df_data = df_data.rename (columns = {"id_expense": "ID Expense"})

    df_data = df_data.reset_index(drop=True).set_index("ID Expense")
    df_data = df_data.sort_values ("date", ascending = True) 

    # Describe dataframe
    st.dataframe(df_data, column_config = {"value": st.column_config.NumberColumn ("Value", format="euro"),
